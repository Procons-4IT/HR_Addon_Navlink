Public Class clshrFunction
    ...
    End Sub
    Public Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_Func) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_hr_Func, frm_hr_Func)
...
#Region "DataBind"
    ...
    Public Sub BindData(ByVal objform As SAPbouiCOM.Form)
        ...
        Try
            ...
            otestrs.DoQuery("Update ""@Z_HR_OFCA"" set ""Name""=' '  where ""Name"" Like '%_XD'")
            oMatrix = objform.Items.Item("3").Specific
            oDBDataSrc = objform.DataSources.DBDataSources.Add("@Z_HR_OFCA")
            Try
                oDBDataSrc.Query()
        ...
        Try
            ...
            For intRow As Integer = aRow To aRow
                If oMatrix.IsRowSelected(intRow) Then
                    ...
                    otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oGeneralService = oCompanyService.GetGeneralService("Z_HR_OFCA")
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("Update ""@Z_HR_OFCA"" set ""Name""=isnull(""Name"",'') + '_XD' where ""Code""='" & strCode & "'")
...
#Region "Enable Matrix After Update"
    ...
    Private Sub EnblMatrixAfterUpdate(ByVal objApplication As SAPbouiCOM.Application, ByVal ocompany As SAPbobsCOM.Company, ByVal oForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            End If
            oUserTable = ocompany.UserTables.Item("Z_HR_OFCA")
            oDBDSource = oForm.DataSources.DBDataSources.Item("@Z_HR_OFCA")
            ...
            For i = 0 To oDBDSource.Size - 1
                ...
                otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                otestRs.DoQuery("SElect * from [@Z_HR_OFCA] where Code='" & strCode & "'")
...
#Region "Insert Code and Doc Entry"
    ...
    Public Sub InsertCodeAndDocEntry(ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            objForm = aForm
            aForm.Freeze(True)
            oDBDSource = objForm.DataSources.DBDataSources.Item("@Z_HR_OFCA")
            objMatrix = objForm.Items.Item("3").Specific
            objMatrix.FlushToDataSource()
            Dim strCode, strDocEntry As String
            strCode = oApplication.Utilities.getMaxCode("@Z_HR_OFCA", "Code")
            strDocEntry = oApplication.Utilities.getMaxCode("@Z_HR_OFCA", "DocEntry")
    ...
    Private Function Validation(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        For intRow As Integer = 1 To oMatrix.RowCount
            ...
            If strcode <> "" And strcode1 = "" Then
                oApplication.Utilities.Message("Enter Function Name...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                oMatrix.Columns.Item("V_1").Cells.Item(oMatrix.RowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                Return False
            End If

            If strcode = "" And strcode1 <> "" Then
                oApplication.Utilities.Message("Enter Function Code...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                oMatrix.Columns.Item("V_0").Cells.Item(oMatrix.RowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                Return False
            End If
        Next
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_Func Then
                Select Case pVal.BeforeAction
                    Case True
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_KEY_DOWN And pVal.CharPressed <> "9" And pVal.ItemUID = "3" And pVal.ColUID = "V_0" Then
                            ...
                            If oApplication.Utilities.ValidateCode(strVal, "FUNC") = True Then
                                BubbleEvent = False
                                Exit Sub
                            End If
                        End If
                    ...
                    Case False
                        If ((pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.ItemUID = "1")) Then
                            ...
                            Dim otestrs As SAPbobsCOM.Recordset
                            otestrs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            otestrs.DoQuery("Delete ""@Z_HR_OFCA"" where ""Name"" Like '%_XD'")
                            objForm = oForm
                            objMatrix = objForm.Items.Item("3").Specific
                            objMatrix.AddRow()
                    ...
                    If pVal.BeforeAction = True Then
                        ...
                        For intRow As Integer = 1 To objMatrix.RowCount
                            If objMatrix.IsRowSelected(intRow) Then
                                ...
                                If oApplication.Utilities.ValidateCode(strValue, "FUNC") = True Then
                                    BubbleEvent = False
                                    Exit Sub
                                Else
