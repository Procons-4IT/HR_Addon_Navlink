Public Class clshrHRTrainApp
    ...
    End Sub
    Public Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_HRTrainApp) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_HRTrainApp, frm_hr_HRTrainApp)
    ...
    Private Sub NewRequestBind(ByVal aform As SAPbouiCOM.Form, ByVal deptno As String)
        ...
        If deptno <> "" Then
            ...
            strqry = strqry & " when 'HA' then 'HR Approved' else 'HR Rejected' end as U_Z_ReqStatus,  case U_Z_mgrstatus when 'P' then 'Pending' when 'MA' then 'Approved' else 'Rejected' end 'U_Z_MgrStatus',U_Z_MgrRemarks,U_Z_HRStatus,"
            strqry = strqry & " U_Z_HRRemarks  from [@Z_HR_ONTREQ] where U_Z_DeptCode='" & deptno & "'" ' and (U_Z_ReqStatus='MA')"
        Else
            strqry = "  select DocEntry,U_Z_ReqDate,U_Z_HREmpID,U_Z_HREmpName,U_Z_CourseName,U_Z_CourseDetails,convert(varchar(10),U_Z_TrainFrdt,103) as U_Z_TrainFrdt,convert(varchar(10),U_Z_TrainTodt,103) as U_Z_TrainTodt,U_Z_TrainCost,U_Z_Notes,"
            strqry = strqry & " U_Z_DeptName,U_Z_PosiName,CASE U_Z_ReqStatus when 'P' then 'Pending' when 'MA' then 'Manager Approved' when 'MR' then 'Manager Rejected'"
            strqry = strqry & " when 'HA' then 'HR Approved' else 'HR Rejected' end as U_Z_ReqStatus,  case U_Z_mgrstatus when 'P' then 'Pending' when 'MA' then 'Approved' else 'Rejected' end 'U_Z_MgrStatus',U_Z_MgrRemarks,U_Z_HRStatus,"
            strqry = strqry & " U_Z_HRRemarks  from [@Z_HR_ONTREQ]" ' where (U_Z_ReqStatus='MA')"
        End If
    ...
    Private Sub ExistingRequestBind(ByVal aform As SAPbouiCOM.Form, ByVal deptno As String)
        ...
        If deptno <> "" Then
            ...
            strqry = strqry & " end as U_Z_MgrStatus,U_Z_MgrRemarks,CASE U_Z_HRStatus when 'P' then 'Pending' when 'HA' then 'HR Approved' when 'HR' then 'HR Rejected'"
            strqry = strqry & " end as U_Z_HRStatus,U_Z_HRRemarks  from [@Z_HR_ONTREQ] where U_Z_DeptCode='" & deptno & "' "
        Else
            strqry = "  select DocEntry,U_Z_ReqDate,U_Z_HREmpID,U_Z_HREmpName,U_Z_CourseName,U_Z_CourseDetails,convert(varchar(10),U_Z_TrainFrdt,103) as U_Z_TrainFrdt,convert(varchar(10),U_Z_TrainTodt,103) as U_Z_TrainTodt,U_Z_TrainCost,U_Z_Notes,"
            strqry = strqry & " U_Z_DeptName,U_Z_PosiName,CASE U_Z_ReqStatus when 'P' then 'Pending' when 'MA' then 'Manager Approved' when 'MR' then 'Manager Rejected'"
            strqry = strqry & " when 'HA' then 'HR Approved' else 'HR Rejected' end as U_Z_ReqStatus,CASE U_Z_MgrStatus when 'P' then 'Pending' when 'MA' then 'Manager Approved' when 'MR' then 'Manager Rejected'"
            strqry = strqry & " end as U_Z_MgrStatus,U_Z_MgrRemarks,CASE U_Z_HRStatus when 'P' then 'Pending' when 'HA' then 'HR Approved' when 'HR' then 'HR Rejected'"
            strqry = strqry & " end as U_Z_HRStatus,U_Z_HRRemarks  from [@Z_HR_ONTREQ]"
        End If
    ...
    Private Function AddToUDT(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        Try
            For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
                ...
                ocom = oGrid.Columns.Item("U_Z_HRStatus")
                If ocom.GetSelectedValue(intRow).Value <> "P" Then
                    strqry = "Update [@Z_HR_ONTREQ] set  U_Z_HRStatus='" & oGrid.DataTable.GetValue("U_Z_HRStatus", intRow) & "',U_Z_HRRemarks='" & oGrid.DataTable.GetValue("U_Z_HRRemarks", intRow) & "',U_Z_ReqStatus='" & oGrid.DataTable.GetValue("U_Z_HRStatus", intRow) & "' where DocEntry='" & strcode & "'"
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_HRTrainApp Then
