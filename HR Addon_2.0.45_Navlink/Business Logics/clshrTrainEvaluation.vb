Public Class clshrTrainEvaluation
    Inherits clsBase
    ...
    Public Sub LoadForm(ByVal EmpCode As String, ByVal EmpName As String)
        'If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_TrainEva) = False Then
        '    oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
        '    Exit Sub
        'End If
        oForm = oApplication.Utilities.LoadForm(xml_hr_TrainEvaluation, frm_hr_TrainEva)
        oForm = oApplication.SBO_Application.Forms.ActiveForm()
        oForm.Freeze(True)
        oApplication.Utilities.setEdittextvalue(oForm, "25", EmpCode)
        oApplication.Utilities.setEdittextvalue(oForm, "27", EmpName)
    ...
    Private Sub Databind(ByVal aform As SAPbouiCOM.Form)
        ...
        strqry = strqry & ",Code from [@Z_HR_TRIN1] where U_Z_HREmpID='" & oApplication.Utilities.getEdittextvalue(aform, "25") & "' and U_Z_AttendeesStatus='C' "
    ...
    Private Sub DatabindEvaluation(ByVal aform As SAPbouiCOM.Form)
        ...
        For introw As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            If oGrid.Rows.IsSelected(introw) Then
                strstring = "select Code,U_Z_EmpCode,U_Z_EmpName,U_Z_AgendaCode,U_Z_QusCatCode,U_Z_QusCatName,U_Z_QusItemCode,U_Z_QusItemName,"
                strstring += " U_Z_QusRatCode,U_Z_QusRatName,U_Z_Comments,U_Z_MgrStatus  from [@Z_HR_TRAEVA]  where U_Z_AgendaCode='" & oGrid.DataTable.GetValue("U_Z_TrainCode", introw) & "' and U_Z_EmpCode='" & oApplication.Utilities.getEdittextvalue(aform, "25") & "'"
                oGrid1.DataTable.ExecuteQuery(strstring)
                FormatGrid(aform, "Lines")
                Exit Sub
            End If
    ...
    Private Sub FormatGrid(ByVal aform As SAPbouiCOM.Form, ByVal aChoice As String)
        ...
        If aChoice = "Header" Then
            ...
            oGrid.Columns.Item("U_Z_CourseName").TitleObject.Caption = "CourseName"
        ...
        ElseIf aChoice = "Lines" Then
            ...
            oEditTextColumn = oGrid1.Columns.Item("U_Z_QusCatCode")
            oEditTextColumn.ChooseFromListUID = "CFL1"
            oEditTextColumn.ChooseFromListAlias = "U_Z_Code"
            ...
            oEditTextColumn = oGrid1.Columns.Item("U_Z_QusItemCode")
            ...
            oEditTextColumn = oGrid1.Columns.Item("U_Z_QusRatCode")
            ...
            oGrid1.Columns.Item("U_Z_MgrStatus").Editable = False
    ...
    Private Sub AddChooseFromList(ByVal objForm As SAPbouiCOM.Form)
        Try
            ...
            oCFLCreationParams.ObjectType = "Z_HR_TRQCCA"
            oCFLCreationParams.UniqueID = "CFL1"
            oCFL = oCFLs.Add(oCFLCreationParams)

            oCFLCreationParams.MultiSelection = False
            oCFLCreationParams.ObjectType = "Z_HR_TRQCIT"
            oCFLCreationParams.UniqueID = "CFL2"
            oCFL = oCFLs.Add(oCFLCreationParams)

            oCFLCreationParams.MultiSelection = False
            oCFLCreationParams.ObjectType = "Z_HR_TRQCRA"
    ...
    Private Sub AddRow(ByVal aForm As SAPbouiCOM.Form)
        ...
        End If
        If oGrid.DataTable.GetValue("U_Z_QusCatCode", oGrid.DataTable.Rows.Count - 1) <> "" Then
            oGrid.DataTable.Rows.Add()
            ...
            End If
        End If
        oGrid.Columns.Item("U_Z_QusCatCode").Click(oGrid.DataTable.Rows.Count - 1)
        oGrid.DataTable.SetValue("U_Z_MgrStatus", oGrid.DataTable.Rows.Count - 1, "P")
    ...
    Private Sub DeleteRow(ByVal aForm As SAPbouiCOM.Form)
        Select Case aForm.PaneLevel
            Case 2
                oGrid = aForm.Items.Item("20").Specific
        End Select
        If oApplication.SBO_Application.MessageBox("Do you want to delete the selected evaluation details?", , "Continue", "Cancel") = 2 Then
            Exit Sub
        End If
        For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
            If oGrid.Rows.IsSelected(intRow) Then
                If oGrid.DataTable.GetValue("U_Z_QusCatCode", oGrid.DataTable.Rows.Count - 1) <> "" Then
                    If oGrid.DataTable.GetValue("U_Z_MgrStatus", intRow) <> "P" Then
                        oApplication.Utilities.Message("Training Evaluation already Approved / Rejected.", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                        Exit For
                    Else
                        Dim otest As SAPbobsCOM.Recordset
                        otest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        otest.DoQuery("Update [@Z_HR_TRAEVA] set Name=Name +'_XD' where Code='" & oGrid.DataTable.GetValue("Code", intRow) & "'")
    ...
    Private Function AddToUDT(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        If strAgendaCode <> "" Then
            For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
                If oUserTable.GetByKey(oGrid.DataTable.GetValue("Code", intRow)) Then
                    ...
                    oUserTable.UserFields.Fields.Item("U_Z_AgendaCode").Value = strAgendaCode
                    oUserTable.UserFields.Fields.Item("U_Z_QusCatCode").Value = oGrid.DataTable.GetValue("U_Z_QusCatCode", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_QusCatName").Value = oGrid.DataTable.GetValue("U_Z_QusCatName", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_QusItemCode").Value = oGrid.DataTable.GetValue("U_Z_QusItemCode", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_QusItemName").Value = oGrid.DataTable.GetValue("U_Z_QusItemName", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_QusRatCode").Value = oGrid.DataTable.GetValue("U_Z_QusRatCode", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_QusRatName").Value = oGrid.DataTable.GetValue("U_Z_QusRatName", intRow)
                ...
                Else
                    ...
                    oUserTable.UserFields.Fields.Item("U_Z_QusRatName").Value = oGrid.DataTable.GetValue("U_Z_QusRatName", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_Comments").Value = oGrid.DataTable.GetValue("U_Z_Comments", intRow)
                    oUserTable.UserFields.Fields.Item("U_Z_MgrStatus").Value = "P"
        ...
        ote.DoQuery("Delete from [@Z_HR_TRAEVA] where Name like '%_XD'")
        Return True
    End Function
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_TrainEva Then
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "5" Then
                                    If oApplication.SBO_Application.MessageBox("Do you want to save the Evaluation Details ? ", , "Continue", "Cancel") = 2 Then
                                        Exit Sub
                                    End If
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        intChoice = 0
                                        oForm.Freeze(True)
                                        If pVal.ItemUID = "20" And pVal.ColUID = "U_Z_QusCatCode" Then
                                            val = oDataTable.GetValue("U_Z_Code", 0)
                                            val1 = oDataTable.GetValue("U_Z_Name", 0)
                                            oGrid = oForm.Items.Item("20").Specific
                                            oGrid.DataTable.SetValue("U_Z_QusCatName", pVal.Row, val1)
                                            oGrid.DataTable.SetValue("U_Z_QusCatCode", pVal.Row, val)
                                        ElseIf pVal.ItemUID = "20" And pVal.ColUID = "U_Z_QusItemCode" Then
                                            val = oDataTable.GetValue("U_Z_Code", 0)
                                            val1 = oDataTable.GetValue("U_Z_Name", 0)
                                            oGrid = oForm.Items.Item("20").Specific
                                            oGrid.DataTable.SetValue("U_Z_QusItemName", pVal.Row, val1)
                                            oGrid.DataTable.SetValue("U_Z_QusItemCode", pVal.Row, val)
                                        ElseIf pVal.ItemUID = "20" And pVal.ColUID = "U_Z_QusRatCode" Then
                                            val = oDataTable.GetValue("U_Z_Code", 0)
                                            val1 = oDataTable.GetValue("U_Z_Name", 0)
                                            oGrid = oForm.Items.Item("20").Specific
                                            oGrid.DataTable.SetValue("U_Z_QusRatName", pVal.Row, val1)
                                            oGrid.DataTable.SetValue("U_Z_QusRatCode", pVal.Row, val)
                                        End If
                                        oForm.Freeze(False)
                                    End If
                                Catch ex As Exception
