Public Class clshrCompany
    ...
    End Sub
    Public Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_Comp) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_hr_Comp, frm_hr_Comp)
...
#Region "DataBind"
    ...
    Public Sub BindData(ByVal objform As SAPbouiCOM.Form)
        ...
        Try
            ...
            otestrs.DoQuery("Update ""@Z_HR_OADM"" set ""Name""=' '  where ""Name"" Like '%_XD'")
            oMatrix = objform.Items.Item("3").Specific
            oDBDataSrc = objform.DataSources.DBDataSources.Add("@Z_HR_OADM")
            Try
                oDBDataSrc.Query()
...
#Region "Enable Matrix After Update"
    ...
    Private Sub EnblMatrixAfterUpdate(ByVal objApplication As SAPbouiCOM.Application, ByVal ocompany As SAPbobsCOM.Company, ByVal oForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            oForm.Freeze(True)
            If oMatrix.Columns.Item(1).Cells.Item(oMatrix.VisualRowCount).Specific.value = "" Then
                oMatrix.DeleteRow(oMatrix.VisualRowCount)
            End If
            oUserTable = ocompany.UserTables.Item("Z_HR_OADM")
            oDBDSource = oForm.DataSources.DBDataSources.Item("@Z_HR_OADM")
            oMatrix.FlushToDataSource()
            For i = 0 To oDBDSource.Size - 1
                oGeneralService = oCompanyService.GetGeneralService("Z_HR_OADM")
                oGeneralData = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData)
                strCode = oDBDSource.GetValue("Code", i).Trim
                otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                otestRs.DoQuery("Select * from [@Z_HR_OADM] where Code='" & strCode & "'")
            ...
            Next
            oDBDSource.Query()
            oMatrix.Columns.Item(1).Cells.Item(oMatrix.VisualRowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
            ' End If
            oForm.Freeze(False)
            Exit Sub
        Catch ex As Exception
...
#Region "Insert Code and Doc Entry"
    ...
    Public Sub InsertCodeAndDocEntry(ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            objForm = aForm
            aForm.Freeze(True)
            oDBDSource = objForm.DataSources.DBDataSources.Item("@Z_HR_OADM")
            objMatrix = objForm.Items.Item("3").Specific
            objMatrix.FlushToDataSource()

            Dim strCode, strDocEntry As String
            strCode = oApplication.Utilities.getMaxCode("@Z_HR_OADM", "Code")
            strDocEntry = oApplication.Utilities.getMaxCode("@Z_HR_OADM", "DocEntry")
            ...
            Else
                ' oDBDSource.SetValue("Code", objMatrix.VisualRowCount - 1, oDBDSource.GetValue("DocEntry", objMatrix.VisualRowCount - 1).PadLeft(8, "0"))
                ' oDBDSource.SetValue("DocEntry", objMatrix.VisualRowCount - 1, oDBDSource.GetValue("DocEntry", objMatrix.VisualRowCount - 1).PadLeft(8, "0"))
                oDBDSource.SetValue("Code", objMatrix.VisualRowCount - 1, strCode)
                oDBDSource.SetValue("DocEntry", objMatrix.VisualRowCount - 1, CInt(strDocEntry))
            End If
...
#End Region
    ...
    Private Function Validation(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        If oMatrix.VisualRowCount > 1 Then
            ...
            strcode = oApplication.Utilities.getMatrixValues(oMatrix, "V_0", oMatrix.VisualRowCount)
            strcode1 = oApplication.Utilities.getMatrixValues(oMatrix, "V_0", oMatrix.VisualRowCount - 1)
            If strcode.ToUpper = strcode1.ToUpper Then
                ...
                oMatrix.Columns.Item("V_0").Cells.Item(oMatrix.VisualRowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                Return False
            End If
        End If
        ...
        For intRow As Integer = 1 To oMatrix.VisualRowCount
            ...
            If strcode <> "" And strcode1 = "" Then
                ...
                oMatrix.Columns.Item("V_1").Cells.Item(oMatrix.VisualRowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                Return False
            End If
        ...
        Try
            ...
            For intRow As Integer = aRow To aRow
                If oMatrix.IsRowSelected(intRow) Then
                    ...
                    oGeneralService = oCompanyService.GetGeneralService("Z_HR_OADM")
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("Update ""@Z_HR_OADM"" set ""Name""=isnull(""Name"",'') + '_XD' where ""Code""='" & strCode & "'")
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_Comp Then
                Select Case pVal.BeforeAction
                    Case True
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_KEY_DOWN And pVal.CharPressed <> "9" And pVal.ItemUID = "3" And pVal.ColUID = "V_0" Then
                            ...
                            If oApplication.Utilities.ValidateCode(strVal, "COMP") = True Then
                                BubbleEvent = False
                                Exit Sub
                            End If
                        End If
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "1" And oForm.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Then
                                    ...
                                    If Validation(oForm) = False Then
                                        ...
                                        Exit Sub
                                    End If
                                    oApplication.Utilities.UpdateObjectName("@Z_HR_OADM", "Z_HR_OADM")
                    ...
                    Case False
                        If ((pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.ItemUID = "1")) Then
                            ...
                            Dim otestrs As SAPbobsCOM.Recordset
                            otestrs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            otestrs.DoQuery("Delete ""@Z_HR_OADM"" where ""Name"" Like '%_XD'")
                            objForm = oForm
                            objMatrix = objForm.Items.Item("3").Specific
                            objMatrix.AddRow()
                    ...
                    If pVal.BeforeAction = True Then
                        ...
                        For intRow As Integer = 1 To objMatrix.RowCount
                            If objMatrix.IsRowSelected(intRow) Then
                                ...
                                If oApplication.Utilities.ValidateCode(strValue, "COMP") = True Then
                                    BubbleEvent = False
                                    Exit Sub
                                Else
