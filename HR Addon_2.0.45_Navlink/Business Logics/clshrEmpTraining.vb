Public Class clshrEmpTraining
    ...
    Private Sub Databind2(ByVal strposcode As String)
        ...
        strqry = strqry & " U_Z_InsName,U_Z_NoOfHours,U_Z_StartTime,U_Z_EndTime,isnull(U_Z_Sunday,'N') 'U_Z_Sunday',isnull(U_Z_Monday,'N') 'U_Z_Monday',"
        strqry += " isnull(U_Z_Tuesday,'N') 'U_Z_Tuesday',isnull(U_Z_Wednesday,'N') 'U_Z_Wednesday',isnull(U_Z_Thursday,'N') 'U_Z_Thursday',isnull(U_Z_Friday,'N') 'U_Z_Friday',"
        strqry += " isnull(U_Z_Saturday,'N') 'U_Z_Saturday',U_Z_AttCost,U_Z_Active,T3.U_Z_TrainLoc AS 'Training Location',T3.U_Z_EstExpe AS 'Travel Expenses'  from [@Z_HR_OTRIN] T0 inner join [@Z_HR_OCOUR] T1 on T0.U_Z_CourseCode=T1.U_Z_CourseCode "
        strqry += " inner join [@Z_HR_COUR4] T2 on T1.DocEntry=t2.DocEntry Left Join [@Z_HR_ONTREQ] T3 On T0.U_Z_NewTrainCode=T3.DocEntry where  (isnull(T1.U_Z_Allpos,'N')='Y' or  T2.U_Z_PosCode='" & strposcode & "') and T0.U_Z_Active='Y' and isnull(T0.U_Z_Status,'O')='O'"
        ...
        oGrid.Columns.Item("U_Z_AttCost").TitleObject.Caption = "Course Cost"
        oGrid.Columns.Item("U_Z_AttCost").Editable = False
            ...
            If oGrid.Rows.IsSelected(intRow) Then
                ...
                If strStatus = "Y" Then
                    ...
                    Else
                        ...
                        If intTempID <> "0" Then
                            oApplication.Utilities.UpdateApprovalRequired("@Z_HR_TRIN1", "Code", strCode, "Y", intTempID)
                            oApplication.Utilities.InitialMessage("Reg.Training Request", strCode, oApplication.Utilities.DocApproval(aForm, HeaderDoctype.Train, strEmpId), intTempID, oApplication.Utilities.getEdittextvalue(aForm, "6"), HistoryDoctype.RegTra)
                        Else
                            oApplication.Utilities.UpdateApprovalRequired("@Z_HR_TRIN1", "Code", strCode, "N", intTempID)
                        End If
