Public Class clshrTrainQcItem
    ...
    Private Sub LoadForm()
        If oApplication.Utilities.validateAuthorization(oApplication.Company.UserSignature, frm_hr_TrainQcItem) = False Then
            oApplication.Utilities.Message("You are not authorized to do this action", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Exit Sub
        End If
        oForm = oApplication.Utilities.LoadForm(xml_HR_TrainQCItem, frm_hr_TrainQcItem)
...
#Region "DataBind"
    ...
    Public Sub BindData(ByVal objform As SAPbouiCOM.Form)
        ...
        Try
            ...
            otestrs.DoQuery("Update ""@Z_HR_TRQCIT"" set ""Name""=' '  where ""Name"" Like '%_XD'")
            oMatrix = objform.Items.Item("3").Specific
            oDBDataSrc = objform.DataSources.DBDataSources.Add("@Z_HR_TRQCIT")
            Try
                oDBDataSrc.Query()
...
#Region "Enable Matrix After Update"
    ...
    Private Sub EnblMatrixAfterUpdate(ByVal objApplication As SAPbouiCOM.Application, ByVal ocompany As SAPbobsCOM.Company, ByVal oForm As SAPbouiCOM.Form)
        ...
        Try
            ...
            End If
            If 1 = 1 Then
                oUserTable = ocompany.UserTables.Item("Z_HR_TRQCIT")
                oDBDSource = oForm.DataSources.DBDataSources.Item("@Z_HR_TRQCIT")
                'oMatrix.DeleteRow(oMatrix.RowCount)
                oMatrix.FlushToDataSource()
                For i = 0 To oDBDSource.Size - 1
                    oGeneralService = oCompanyService.GetGeneralService("Z_HR_TRQCIT")
                    oGeneralData = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData)
                    strCode = oDBDSource.GetValue("Code", i).Trim
                    If 1 = 1 Then ' strCode <> "" Then


                        otestRs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                        otestRs.DoQuery("SElect * from [@Z_HR_TRQCIT] where Code='" & strCode & "'")
                        If otestRs.RecordCount > 0 Then
                            ...
                            oGeneralData.SetProperty("U_Z_Name", oDBDSource.GetValue("U_Z_Name", i))
...
#Region "Insert Code and Doc Entry"
    ...
    Public Sub InsertCodeAndDocEntry(ByVal aForm As SAPbouiCOM.Form)
        ...
        Try
            objForm = aForm
            aForm.Freeze(True)
            oDBDSource = objForm.DataSources.DBDataSources.Item("@Z_HR_TRQCIT")
            objMatrix = objForm.Items.Item("3").Specific
            objMatrix.FlushToDataSource()
            Dim strCode, strDocEntry As String
            strCode = oApplication.Utilities.getMaxCode("@Z_HR_TRQCIT", "Code")
            strDocEntry = oApplication.Utilities.getMaxCode("@Z_HR_TRQCIT", "DocEntry")
...
#End Region
    ...
    Private Function Validation(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        For intRow As Integer = 1 To oMatrix.RowCount
            ...
            '    If oMatrix.RowCount = 1 Then
            '        If strcode = "" And strcode1 = "" Then
            '            oApplication.Utilities.Message("Code / Name can not be empty...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            '            oMatrix.Columns.Item("V_0").Cells.Item(oMatrix.RowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
            '            Return False
            '        End If
            '    End If
        ...
        Try
            ...
            For intRow As Integer = aRow To aRow
                If oMatrix.IsRowSelected(intRow) Then
                    ...
                    oGeneralService = oCompanyService.GetGeneralService("Z_HR_TRQCIT")
                    oGeneralParams = oGeneralService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams)
                    otestRs.DoQuery("Update ""@Z_HR_TRQCIT"" set ""Name""=isnull(""Name"",'') + '_XD' where ""Code""='" & strCode & "'")
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_hr_TrainQcItem Then
                Select Case pVal.BeforeAction
                    Case True
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_KEY_DOWN And pVal.CharPressed <> "9" And pVal.ItemUID = "3" And pVal.ColUID = "V_0" Then
                            ...
                            If strVal <> "" Then
                                'If oApplication.Utilities.ValidateCode(strVal, "Z_HR_TRQCIT") = True Then
                                '    BubbleEvent = False
                                '    Exit Sub
                                'End If
                            End If
                    ...
                    Case False
                        If ((pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED) And (pVal.ItemUID = "1")) Then
                            ...
                            Dim otestrs As SAPbobsCOM.Recordset
                            otestrs = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                            otestrs.DoQuery("Delete ""@Z_HR_TRQCIT"" where ""Name"" Like '%_XD'")
                            objForm = oForm
                            oMatrix = objForm.Items.Item("3").Specific
                            Dim oDBDataSrc As SAPbouiCOM.DBDataSource
                            oDBDataSrc = objForm.DataSources.DBDataSources.Item("@Z_HR_TRQCIT")
                            Try
                                oDBDataSrc.Query()
                            Catch ex As Exception
                            ...
                            oMatrix.LoadFromDataSource()
                            If 1 = 1 Then
                                If oDBDataSrc.Size <= 0 Then
                                    oMatrix.AddRow()
                                ElseIf oDBDataSrc.GetValue("U_Z_Code", oDBDataSrc.Size - 1) <> "" Then
                                    oMatrix.AddRow()
                                End If
                                If 1 = 1 Then ' oDBDataSrc.GetValue("U_Z_Code", oDBDataSrc.Size - 1) <> "" Then
                                    ' oMatrix.AddRow()
                                    objMatrix.Columns.Item(0).Cells.Item(oMatrix.RowCount).Specific.value = objMatrix.RowCount
                                    objMatrix.Columns.Item("V_0").Cells.Item(oMatrix.RowCount).Specific.value = ""
                                    objMatrix.Columns.Item("V_1").Cells.Item(oMatrix.RowCount).Specific.value = ""
                                    oCombobox = objMatrix.Columns.Item("V_2").Cells.Item(oMatrix.RowCount).Specific
                                    oCombobox.Select("Y", SAPbouiCOM.BoSearchKey.psk_ByValue)
                                End If
                                oApplication.Utilities.AssignSerialNo(oMatrix, oForm)
                            End If
                            oMatrix.Columns.Item(1).Cells.Item(oMatrix.RowCount).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
